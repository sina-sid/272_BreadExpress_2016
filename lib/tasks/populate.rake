namespace :db do
  ...
  # Step 6: Create a known customer
    ted_user = FactoryGirl.create(:user, username: "tgruberman", password: "bread", password_confirmation: "bread")
    ted = FactoryGirl.create(:customer, user: ted_user, first_name: "Ted", last_name: "Gruberman")
    ted_credit_card = credit_cards.sample
    billing = FactoryGirl.create(:address, customer: ted, 
      recipient: "#{ted.proper_name}",
      street_1: "#{Faker::Address.street_address}",
      city: "#{Faker::Address.city}",
      state: "#{Address::STATES_LIST.to_h.values.sample}",
      zip: "#{rand(100000).to_s.rjust(5,"0")}",
      is_billing: true)
    address_2 = FactoryGirl.create(:address, customer: ted,
      recipient: "Kathryn Janeway", 
      street_1: "#{Faker::Address.street_address}",
      city: "#{Faker::Address.city}",
      state: "#{Address::STATES_LIST.to_h.values.sample}",
      zip: "#{rand(100000).to_s.rjust(5,"0")}")          
    address_3 = FactoryGirl.create(:address, customer: ted, 
      recipient: "Ben Sisko",
      street_1: "#{Faker::Address.street_address}",
      street_2: "#{Faker::Address.secondary_address}",
      city: "#{Faker::Address.city}",
    ...
    c_address_ids = [billing.id, address_2.id, address_3.id]
    customer_selections = all_items.shuffle
    12.times do |i|
      order = Order.new
      order.customer_id = ted.id
      order.address_id = c_address_ids.sample
      order.date = (5.months.ago.to_date..2.days.ago.to_date).to_a.sample
      order.save!
      total = 0
      ...
      order.update_attribute(:grand_total, total)
      # set credit card info
      order.credit_card_number = ted_credit_card.number
      order.expiration_year = ted_credit_card.expiration_year
      order.expiration_month = ted_credit_card.expiration_month
      # pay
      order.pay
      # ship the items
      order.order_items.each{|oi2| oi2.shipped_on = order.date + 1; oi2.save! }
